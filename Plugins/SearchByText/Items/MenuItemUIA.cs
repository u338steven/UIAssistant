                    ...
                    observer.Callback += x =>
                    {
                        try
                        {
                            if (Ancestors.Count == 0)
                            {
                                ...
                                if (element == null)
                                {
                                    UIAssistantAPI.NotifyWarnMessage(Consts.PluginName, "Cannot find the selected menu item");
                                    return;
                                }
                                ...
                                {
                                    UIAssistantAPI.NotifyWarnMessage(Consts.PluginName, "Cannot run the selected menu item");
                                    return;
                        ...
                        {
                            UIAssistantAPI.NotifyErrorMessage(Consts.PluginName, ex.Message + "\nThe menu may not be scrolled");
                        }
                        finally
                        {
                            if (shouldBeClosed)
